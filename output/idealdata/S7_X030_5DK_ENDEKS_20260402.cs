...
// Layer 2
float ST_FACTOR = 3.0f;
int ST_HHV_PERIOD = 10; // ATR Period in Python 
int ST_ATR_PERIOD = 14;
int EMA_FAST_PERIOD = 9;
int EMA_SLOW_PERIOD = 21;
// Layer 3
int TOMA_PERIOD = 1;
float TOMA_OPT = 2.1f;
int HHV_TETIK_PERIOD = 12;
int LLV_TETIK_PERIOD = 12;
// Layer 4
int MFI_PERIOD = 14;
int MFI_HHV_PERIOD = 5;
int MFI_LLV_PERIOD = 5;
float MFI_LONG = 55.0f;
float MFI_SHORT = 45.0f;
float VOL_RATIO = 0.8f;
// Layer 5
int ATR_PERIOD = 14;
float ATR_STOP_MULT_LONG = 1.5f;
float ATR_STOP_MULT_SHORT = 1.5f;
float KAR_AL_YUZDE_LONG = 2.0f;
float KAR_AL_YUZDE_SHORT = 2.0f;
// Layer 6
int MIN_HOLD_BARS = 2;
...
for (int i = 0; i < V.Count; i++) {
    ...
    ARS_Band[i] = Sistem.SayiYuvarla(ARS_Band[i], 0.01f);
}
...
TR[0] = H[0] - L[0];
var ATR_ST = Sistem.Liste(0);
float alpha = 1.0f / ST_ATR_PERIOD; // Python uses ST_HHV_PERIOD essentially for ATR
...
for (int i = 1; i < V.Count; i++) {
    ...
    if (i == 1) {
        ...
        ATR_ST[i] = TR[i] * alpha + ATR_ST[i-1] * (1.0f - alpha);
    }
}

var ST_Up = Sistem.Liste(0);
var ST_Dn = Sistem.Liste(0);
var ST = Sistem.Liste(0);
int st_trend = -1;

for (int i = 1; i < V.Count; i++) {
    float mid = (H[i] + L[i]) / 2.0f;
    ST_Up[i] = mid + ST_FACTOR * ATR_ST[i];
    ST_Dn[i] = mid - ST_FACTOR * ATR_ST[i];

    if (i == 1) {
        ST[i] = ST_Up[i];
    } else {
        if (st_trend == 1) { // Uptrend, trail lower band
            ST[i] = Math.Max(ST_Dn[i], ST[i-1]);
            if (C[i] < ST[i]) {
                st_trend = -1;
                ST[i] = ST_Up[i];
            }
        } else { // Downtrend, trail upper band
            ST[i] = Math.Min(ST_Up[i], ST[i-1]);
            if (C[i] > ST[i]) {
                st_trend = 1;
                ST[i] = ST_Dn[i];
            }
...
var VOL_MA = Sistem.MA(Lot, "Simple", 20);
...
for (int i = warmBars; i < V.Count; i++)
{
    ...
    bool vol_ok = (Lot[i] >= VOL_MA[i-1] * VOL_RATIO);
