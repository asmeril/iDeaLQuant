...
// Mod: VIOP — Çift Yön (ENDEKS)
// Otomatik: IdealQuant Export | Tarih: 2026-03-08 05:06
// ===============================================================================================
// Vadeli: Çift Yönlü
...
// === PARAMETRELER ===
int OTT_Period = 20;
float OTT_Pct_Big = 6.5f;
float OTT_Pct_Small = 3.1999999999999997f;
float OTT_Mult = 0.0005f;
float SOTT_Pct = 0.4f;
int Gate_Period = 10;
float Gate_Pct = 0.4f;
int Stoch_K = 500;
int Stoch_Smooth = 200;
...
var HHV_Half = Sistem.HHV(5, "Yuksek");
var LLV_Half = Sistem.LLV(5, "Dusuk");
...
for (int i = warmupBars; i < Veriler.Count; i++)
{
    ...
    else if (sat && SonYon != "S" && YON_MODU != "SADECE_AL")
    {{
        Sinyal = "S";
    }}
    else if (SonYon == "A" && !al)
